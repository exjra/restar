...
//    return app.exec();
    ...
    while(key != 'q')				// While loop to query for Image Input frame
    {
        ...
        if (mark >= 3)		// Ensure we have (atleast 3; namely A,B,C) 'Alignment Markers' discovered
        {
            ...
            if( top < contours.size() && right < contours.size() && bottom < contours.size() && contourArea(contours[top]) > 10 && contourArea(contours[right]) > 10 && contourArea(contours[bottom]) > 10 )
            {
                ...
                if (src.size() == 4 && dst.size() == 4 )			// Failsafe for WarpMatrix Calculation to have only 4 Points with src and dst
                {
                    ...
                    cameraPoseFromHomography(warp_matrix, tPose);
        ...
        if(tFoundQr)
        {
            ...
            imshow ( "QR gray", qr_gray);

            QZXing* qr = new QZXing(QZXing::DecoderFormat_QR_CODE);

            QImage tQrImg(qr_gray.cols, qr_gray.rows, QImage::Format_Grayscale8);

            qDebug()<<"Size Control"<<qr_gray.cols*qr_gray.rows*qr_gray.channels()<<tQrImg.byteCount();

            memcpy(tQrImg.bits(), qr_gray.data, qr_gray.cols*qr_gray.rows*qr_gray.channels());

    //        qDebug()<<"Save path:"<<QApplication::applicationDirPath() + "/qr.jpg";
    //        tQrImg.save(QApplication::applicationDirPath() + "/qr.jpg");

            QString tDecodedString = qr->decodeImage(tQrImg);
            qDebug()<<"Qr Code"<<tDecodedString;
            ...
            for(int t = 0; t<3; t++)
            {
                ...
                for(int y = 0; y<3; y++)
                {
                    tOut += QString::number(warp_matrix.at<float>(t,y)) + "     ";
                }
