...
//        __BEGIN__
        if( adaptive_method != CV_ADAPTIVE_THRESH_MEAN_C &&
            ...
            qDebug()<<"ERROR:"; //CV_ERROR( CV_StsBadArg,
...
//                      "adaptive method are acceptable" );
        ...
        if( threshold_type != CV_THRESH_BINARY &&
            ...
            qDebug()<<"ERROR:"; //CV_ERROR( CV_StsBadArg, "Only CV_TRESH_BINARY and CV_THRESH_BINARY_INV "
//                      "threshold types are acceptable" );
        ...
        if( !CV_ARE_CNS_EQ( srcMat, dstMat ))
            qDebug()<<"ERROR:"; //CV_ERROR( CV_StsUnmatchedFormats, "" );
        
        if( CV_MAT_TYPE(dstMat->type) != CV_8UC1 )
            qDebug()<<"ERROR:"; //CV_ERROR( CV_StsUnsupportedFormat, "" );
        
        if( !CV_ARE_SIZES_EQ( srcMat, dstMat ) )
            qDebug()<<"ERROR:"; //CV_ERROR( CV_StsUnmatchedSizes, "" );
